<script lang="ts">
    ...
    const components = [
        {
            title: 'Recovery',
            description: 'A dual-deployment recovery system provides a controlled descent, ensuring safe recovery of the vehicle.'
        },
        {
            title: 'Avionics',
            description: 'Custom flight hardware and software enable 1 kHz sensor data logging and real-time telemetry, facilitating mission-critical monitoring and control of the vehicle.'
        },
        {
            title: 'Pressurant',
            description: 'A high-pressure nitrogen system, regulated down from 6000 psi, is used to maintain constant propellant pressure throughout the engine burn.'
        },
        {
            title: 'Propellant',
            description: 'Bioethanol and nitrous oxide are stored in a tube-in-tube tank, minimizing external plumbing and leading to a significant reduction in overall vehicle length.'
        },
        {
            title: 'Engine',
            description: 'Propellants are introduced to the combustion chamber through a metal 3D-printed injector manifold and a custom-designed injector. The engine generates 5 kN (1,100 lbf) of force, carrying the vehicle to its target altitude of 10,000 ft (3048 m).'
        }
    ];
</script>
...
<div class="page">
    ...
    <section class="relative bg-gradient-to-b from-surface-700 via-surface-800 to-surface-900 overflow-hidden min-h-screen flex items-center justify-center">
        <div class="absolute -top-20 -right-28 w-[300px] md:w-[500px] h-[700px] bg-gradient-to-br from-transparent via-transparent to-primary-900/10 -rotate-15 pointer-events-none"></div>
			<div class="relative z-10 max-w-[1200px] mx-auto px-6 md:px-12 pt-24 pb-10 md:py-10 w-full box-border">            
				<div class="mb-12">
                <div class="mb-4">
                    <!-- Responsive heading size -->
                    <h1 class="text-5xl md:text-8xl font-bold text-white mt-2 mb-3.5">Proteus</h1>
                    <!-- Fixed-width underline capped so it doesn't overflow on mobile -->
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-[2px]">
                    ...
                    <!-- Description Text -->
                    <div
                        class="bg-surface-900 p-6 md:p-9 flex flex-col gap-4 relative overflow-hidden transition-colors duration-200
    ...
    >
        <div class="relative z-10 max-w-[1200px] mx-auto px-4 md:px-12 py-8 md:py-10 w-full box-border">
            <div class="mb-10">
                <!-- Responsive heading size -->
                <h1 class="text-4xl md:text-7xl font-bold text-white mt-2 mb-3.5">Specifications</h1>
                <div class="w-full max-w-[520px] h-[3px] bg-primary-500"></div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-[2px] bg-primary-700/40 mb-[32px]">
                <div class="bg-surface-900 p-5 md:p-8 flex flex-col gap-2">
                    <!-- Responsive stat number size -->
                    <div class="text-4xl md:text-6xl font-bold text-white">10,000 ft</div>
    ...
    <section
        class="page min-h-screen py-20 bg-surface-900 overflow-hidden hero-section relative flex items-center justify-center"
    >
        <div class="max-w-6xl mx-auto px-4 md:px-2 w-full">
            <div class="mb-12 md:mb-32 relative z-10">
                <h2 class="text-4xl md:text-7xl font-bold text-[--base-font-color] mt-2 mb-3.5">Architecture</h2>
                <div class="w-full max-w-[480px] h-[3px] bg-primary-500"></div>
            </div>

            <!-- Desktop: interactive diagram (hidden on mobile) -->
            <div class="hidden md:block relative flex justify-center items-center w-full" style="height: 1000px;">
                <!-- Rocket Image -->
                <img
                    src={proteusPhoto}
            ...
            <!-- Mobile: rocket image + stacked component cards -->
            <div class="md:hidden flex flex-col gap-6">
                <img
                    src={proteusPhoto}
                    alt="Proteus Architecture"
                    class="w-full max-h-[340px] object-contain"
                />
                <div class="flex flex-col gap-[2px]">
                    {#each components as component}
                        <div
                            class="bg-surface-900 p-5 flex flex-col gap-1 relative overflow-hidden
                                transition-colors duration-200
                                before:absolute before:top-0 before:left-0 before:w-[3px] before:h-full before:bg-primary-500"
                        >
                            <h4 class="font-bold text-white text-base">{component.title}</h4>
                            <p class="text-white/70 text-sm leading-6">{component.description}</p>
                        </div>
                    {/each}
                </div>
            </div>
