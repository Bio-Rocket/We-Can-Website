<script lang="ts">
	...
	> = {
		gold: {
			...
			padClass: 'p-7 sm:p-8',
			logoClamp: 'clamp(120px, 8vw, 170px)',
		},
		silver: {
		...
		copper: {
			...
			blurb: 'Valued partners supporting the mission.',
	...
	function groupByTier(list: typeof sponsors) {
		...
		for (const s of list) {
			const tier = (tierOrder.includes(s.tier as any) ? (s.tier as TierKey) : 'copper') as TierKey;
			map.get(tier)!.push(s);
		}
		return map;
	}
	...
	<div class="relative z-10 max-w-[1200px] mx-auto px-4 md:px-12 py-8 md:py-10 w-full box-border">
		<div class="mb-10">
			...
			<p class="text-base leading-7 text-white/80 max-w-3xl mt-6">
				We’re grateful to the organizations supporting the Western Canada Aerospace Initiative. Want
				to partner with us? Check out <a
					href="/get-involved"
					class="text-primary-400 hover:underline">Get Involved</a
				>.
			</p>
		</div>
			...
			{#if items.length > 0}
				...
				<section class="mb-12">
					<div class="flex flex-col md:flex-row md:items-end md:justify-between gap-3 mb-5">
						<div>
							<h2 class="text-[clamp(1.75rem,3.2vw,2.5rem)] font-bold text-white m-0">
								{meta.title}
							</h2>
						...
						<span
							class="w-fit text-xs text-white/60 uppercase tracking-widest border border-primary-700/40 bg-surface-900 px-3 py-2"
						>
							{tier}
						</span>
					</div>
		...
		>
			...
			<div class="mt-6 flex flex-col sm:flex-row gap-3 sm:gap-4">
				...
				<a
					...
					href="mailto:info@wcan-aero.ca?subject=WCAN%20Sponsorship%20Inquiry"
				>
					Contact us
				</a>
			</div>
...
<style>
	...
	.sponsor-tile:hover .logo-black {
		opacity: 0;
	}
	.sponsor-tile:hover .logo-color {
		opacity: 1;
	}
</style>
